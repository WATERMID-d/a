...
def captcha_bypass(token, url, key, captcha_rqdata):
    ...
    with open("config.json") as dsc_ez:
        ...
        capmonster = HCaptchaTask(config['c90d33c787fa6d7fa9d557bad3ac748b1'])
...
def join(token):
    ...
    with open("config.json") as jn:
        check = json.load(jn)
        invite_code = check.get('https://discord.gg/EfphABYT')
    ...
    if response.status_code == 400:
        ...
        if newresponse.status_code == 200:
            ...
            update_console_title()
            b = newresponse.json()
            server_id = b["Skimbiditoilet69's server"]["1323292064288342036"]
